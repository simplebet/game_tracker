...
class PlayRibbonComponentMinimized extends PositionComponent
    with HasGameRef, Completable {
  PlayRibbonComponentMinimized({
    required this.color,
    required this.text,
  ...
  void render(Canvas canvas) {
    ...
    const double borderStrokeWidth = 1.5;
    ...
    const double ribbonHeight = kPassArrowHeightMinimized;
  ...
  Vector2 _calculateComponentPosition(
    ...
    if (direction == HomeOrAway.home) {
      baseX = (isEnd
              ? getEndXPositionMinimized(yardline, side, screenWidth)
              : getXPositionMinimized(yardline, side, screenWidth)) -
          triangleWidth -
          rectangleWidth +
          borderStrokeWidth -
          widthPadding;
    ...
    return Vector2(
        baseX + (direction == HomeOrAway.home ? -offsetX : offsetX), 0);
  }
